(() => {
  ...
  const PROMPTS = [
    ...
    },
    {
      ...
      payloadTemplate: {
        ...
        branch: "main",
        commitMessage: "Deploy agent via CANARIA",
      },
      badge: "GitHub",
  ...
  async function pushWorkerToGithub(agent, flow, payload) {
    ...
    const message = String(payload.commitMessage || "Deploy agent via CANARIA").trim() || "Deploy agent via CANARIA";
    const workerName = sanitizeWorkerName(payload.workerName || agent?.name || "canaria-agent");
