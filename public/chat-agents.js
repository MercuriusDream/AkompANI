(() => {
  ...
  const STORAGE = {
    ...
    localThreads: "akompani_local_chat_threads",
    localMessages: "akompani_local_chat_messages",
    chatMode: "akompani_chat_runtime_mode",
  ...
  async function localByokReply(agent, threadId, text) {
    ...
    if (!cfg.apiKey || !cfg.endpoint || !cfg.model) {
      throw new Error("LLM not configured. Go to Settings and set up a provider with an API key.");
    }
    ...
    const systemPrompt = [
      "You are Akompani Agent Console Assistant for deployed/runtime validation.",
      "Priority: diagnose root cause first, then provide concrete corrective actions.",
      "Support OpenAI-style compatibility (messages/tools/stream) and deployed-worker troubleshooting.",
      ...
      "If assumptions are uncertain, ask one precise clarifying question.",
      "Keep responses concise, specific, and implementation-ready.",
    ].join(" ");
    ...
    try {
      ...
      if (err.name === "AbortError") throw new Error("Request timed out (45s). Try again.");
  ...
  async function readOpenAiSseResponse(response) {
    ...
    if (toolCalls.length) {
      return `${content ? `${content}\n\n` : ""}[tool_calls] ${stringify(toolCalls)}`.trim();
    }
    return content.trim();
  }
  ...
  async function cloudflareReply(agent, deployment, threadId, text) {
    ...
    const history = buildMessageHistory(threadId);
    const controller = new AbortController();
    const cfTimeoutId = setTimeout(() => controller.abort(), 60000);
    const workerToken = getWorkerAuthToken();
    const headers = {
    ...
    if (contentType.includes("text/event-stream")) {
      ...
      const streamed = await readOpenAiSseResponse(response);
      return streamed || "Worker replied with an empty stream.";
    }
    ...
    const openAiLike = parseChatCompletionsContent(body);
    if (openAiLike) {
      return openAiLike;
    }
