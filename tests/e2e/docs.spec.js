...
test("docs landing and first tutorial render", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Build accompanying AI agents with confidence." })).toBeVisible();
  await page.getByRole("link", { name: "First Agent in 5 Minutes" }).click();
